...
Thread::Thread(const char* threadName,int mypriority , bool wait)
{
    effectivePriority = realPriority = mypriority;
    joinOption = wait;

#ifdef USER_PROGRAM
    space = NULL;
    myPid = processTable->NewEntry(this);
    if (myPid==-1)
        throw 666;
...
Thread::~Thread()
{
    ...
    if (space!=NULL)
        delete space;
    processTable->DeleteEntry(myPid);
#endif
...
//
void
Thread::Finish (int exitValue)
{
    if (joinOption)
        joinPort->Send(exitValue);
