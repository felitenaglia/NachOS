...
class Port; // Resuelve la dependencia cíclicas
...
class Thread {
  ...
  public:
    Thread(const char* debugName,int mypriority=0, bool wait=true);	// initialize a Thread 
    ...
    void Finish(int exitValue = 0);  				// The thread is done executing
    ...
    int GetPriority() {return effectivePriority;}
    void SetPriority(int newPriority) { effectivePriority = newPriority; }
    void RestorePriority() { effectivePriority=realPriority; }
  ...
  private:
    ...
    Port* joinPort;
    bool joinOption;
    int realPriority, effectivePriority;
