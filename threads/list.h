...
class List {
  public:
    ...
    Item RemoveOnce(Item itemToRemove);
    void ApplyWithContext(void (*func)(Item, void *c),void* context);
...
/*
   Borra exactamente un elemento igual al elemento pasado por argumento
   Si la lista es vacía, falla.
*/

template <class Item>
Item
List<Item>::RemoveOnce(Item itemToRemove){
    ListNode *element,*predecesor;
    for (element = first,predecesor=NULL; element!=NULL ; predecesor=element,element=element->next)
	if (element->item == itemToRemove) {
		...
		if (element==last)
			last = predecesor;
		if (predecesor!=NULL) 
			predecesor->next = element->next;
                break;
	}
   return element==NULL ? Item() : element->item;
}

template <class Item>
void
List<Item>::ApplyWithContext(void (*func)(Item,void*),void* context)
{
    for (ListNode *ptr = first; ptr != NULL; ptr = ptr->next) {
       func(ptr->item,context);
    }
}
